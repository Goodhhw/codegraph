...
| UnrealEngine (≥25K tier, measured) | 100956 | 5 (ceiling; overview Q used 3) | 24K | 7000 |

The ~20k/~40k rows are **extrapolated**, not measured. The only measured ≥25K-tier repo is UnrealEngine, and there the 5-call budget must be read as a **ceiling**: on an overview question ("tell me about the camera system") the agent spent 3 calls where 1–2 sufficed and the with-codegraph arm cost **26% more** than plain Read/Grep, because each ~24K-char call is ~7K tokens of prompt-cache *write* (20× a cache read) and a simple question doesn't grow with the repo. The budget wording now states a ceiling ("up to N; most questions need 1–2"). Analysis + measurements: `docs/design/large-repo-simple-question-cost.md`.
...
4. **Pass bar:** a normal flow question reaches **~0 Read/Grep within the repo's explore-call budget**, runs **faster** than without-codegraph, and shows **no regression on a control repo**. Record the numbers in `docs/design/dynamic-dispatch-coverage-playbook.md` (the coverage matrix). **On a ≥25K-file repo, also run one *simple* question (overview / single symbol):** its `cache_creation + output` cost in the with-arm must not exceed the without-arm's, and the explore-call median must be ≤ 2. Read those buckets off `parse-run.mjs`'s `billed buckets` line / `compare-arms.mjs`'s bucket rows (`scripts/agent-eval/cost-buckets.mjs`), **not** off total tokens — the total is dominated by cheap cache reads and structurally hides a write-cost gap. Two runs sharing a prompt prefix inside one 1-hour cache window make the second one cheaper (its first turn becomes a cache read), so either trust only each arm's first run or separate the arms' windows.
